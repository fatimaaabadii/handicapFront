...
export function DataTable({
  ...
  const [sorting, setSorting] = React.useState([{ id: 'id', desc: true }]); // Default sort by 'id' descending

  const [columnFilters, setColumnFilters] = React.useState([]);
  const [columnVisibility, setColumnVisibility] = React.useState({ id: false }); // Hide 'id' column by default
  ...
  return (
    <div className="w-full my-6" style={{ background: '#fff', padding: '20px' }}>
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} style={{ backgroundColor: 'transparent' }}>
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead key={header.id} style={{ color: '#333', fontSize: '16px', fontFamily: 'Nunito, sans-serif', padding: '12px 20px', height: '50px', textAlign: 'right', }}>
